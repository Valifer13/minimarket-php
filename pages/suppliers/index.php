...
$query = "SELECT * FROM suppliers ORDER BY status < 1";
$result = mysqli_query($conn, $query);
?>

<div id="name-page" data-page="suppliers" class="hidden"></div>

<!-- Page title -->
<div class="flex justify-between items-center mb-4">
    <h1 class="text-3xl font-medium tracking-tighter">Suppliers List</h1>
    <div class="flex gap-2 md:gap-4">
        ...
        <a href="<?= BASEURL ?>/pages/suppliers/add.php" class="flex gap-2 px-3 py-2 bg-blue-500 rounded-md items-center text-white text-sm tracking-tight cursor-pointer hover:bg-blue-600 transition-all duration-300 font-medium border-b-5 border-b-blue-800">
            ...
            <span class="hidden md:block">ADD SUPPLIER</span>
        </a>
    </div>
</div>
...
<div class="flex flex-col-reverse md:flex-row justify-end md:justify-between gap-4 md:gap-0 items-end p-1 mb-4">
    <form action="" method="post" class="flex items-center p-1 w-full md:w-xs">
        <div class="p-2 border border-zinc-400 bg-zinc-100 rounded-s-md text-sm w-2xs flex gap-2">
            ...
            <input class="w-full focus:outline-none" type="text" name="search" id="search-keyword" placeholder="Search supplier...">
        </div>
    ...
    <table class="table-auto w-full min:w-max">
        <thead>
            <tr class="text-zinc-400 *:text-sm *:text-start">
                ...
                <th class="border-b border-b-zinc-300 p-2">Contact</th>
        ...
        <tbody id="suppliers-table-body">
            ...
            ?>
                <tr class="**:text-sm **:text-nowrap">
                    ...
                    <?php endif ?>
                    <td class="border-y border-y-zinc-300 p-2"><?= $row['contact'] ?></td>
